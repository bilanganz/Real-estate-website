...
// && $_SESSION['user_type']!=PENDING.AGENT
if($_SESSION['user_type']!=CLIENT && $_SESSION['user_type']!=ADMIN && $_SESSION['user_type']!=AGENT){
...
if($_SERVER['REQUEST_METHOD'] == "GET"){
	$record_result = pg_execute($connect, 'users_persons_select', array($user_id));
	$row = pg_fetch_assoc($record_result);
	
	//$user_id = "";//In the session
	$user_type=$row["user_type"];//default
	$email_address=$row["email_address"];
	$salutations = $row["salutation"];
    ...
    */
	...
	if ($error == "") {
		...
		//$update_result = pg_execute($connect, 'users_update', array($user_type,$email_address,$user_id));
		$update_result = pg_execute($connect, 'users_update', array($email_address,$user_id));
		$update_result = pg_execute($connect, 'persons_update', array($salutations,$first_name,$last_name,$street_address1,$street_address2,$city,$province,$postal_code,$primary_phone_number,$secondary_phone_number,$fax_number,$preferred_contact_method,$user_id));
		echo ' Update ' . $user_id . 'successful';

		$_SESSION['unauthorized_access'] = "Update success !";
...
<div id="bg">
	...
	<table align="center">
		...
		<!--<tr>
			<td><label for="user_type">You currently register as:</label></td>
			<td>
				<?php if($user_type==ADMIN) {?> ADMIN <?php } else if ($user_type==PENDING.AGENT) {?> PENDING AGENT <?php } else { ?>
					<input type="radio" name="user_type" value=<?php echo CLIENT; ?> <?php if(strpos($user_type,CLIENT)||$user_type==CLIENT) {?> checked="checked" <?php } ?>/> CLIENT
					<input type="radio" name="user_type" value=<?php echo AGENT; ?> <?php if(strpos($user_type,AGENT)||$user_type==AGENT) {?> checked="checked" <?php } ?> /> AGENT
				<?php } ?>
		...
		<tr>
    			<td colspan="2" align="center"><input type="submit" name="submit" class="bt" value="Update"/>
